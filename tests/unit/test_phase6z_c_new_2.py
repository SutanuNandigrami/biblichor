...
def _make_db_with_books(tmp_path):
    ...
    conn.execute(
        "INSERT INTO books (id, title, created_at, updated_at) VALUES (1, 'Old Book', '2026-01-01 00:00:00', '2026-01-01 00:00:00')"
    )
    conn.execute(
        "INSERT INTO books (id, title, created_at, updated_at) VALUES (2, 'Mid Book', '2026-03-15 12:00:00', '2026-03-15 12:00:00')"
    )
    conn.execute(
        "INSERT INTO books (id, title, created_at, updated_at) VALUES (3, 'New Book', '2026-05-20 08:00:00', '2026-05-20 08:00:00')"
    )
    conn.commit()
    conn.close()
    return db_path
...
def test_t_separator_matches_zero_rows(tmp_path):
    ...
    n = conn.execute("SELECT COUNT(*) FROM books WHERE created_at >= ?", (bad_date,)).fetchone()[0]
    conn.close()
