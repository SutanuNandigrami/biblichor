...
def test_run_bench_records_timeout_as_failure(tmp_path, monkeypatch):
    ...
    cfg = types.SimpleNamespace(
        ...
        bench=types.SimpleNamespace(
            per_query_timeout_sec=0.5, circuit_break_after_consecutive_fails=3
        ),
    )
...
def test_run_bench_circuit_breaks_after_3_consecutive_fails(tmp_path, monkeypatch):
    ...
    cfg = types.SimpleNamespace(
        ...
        bench=types.SimpleNamespace(
            per_query_timeout_sec=5, circuit_break_after_consecutive_fails=3
        ),
    )
...
def _app(tmp_path: Path) -> FastAPI:
    ...
    cfg = SimpleNamespace(
        ...
        bench=SimpleNamespace(per_query_timeout_sec=20, circuit_break_after_consecutive_fails=3),
