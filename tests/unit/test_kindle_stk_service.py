...
def test_start_oauth_returns_url_and_stashes_verifier(fake_bookorbit_svc, monkeypatch):
    ...
    assert (
        fake_bookorbit_svc.get_secret_value("kindle_stk.oauth_state.code_verifier")
...
def test_complete_oauth_extracts_code_and_persists_cert(fake_bookorbit_svc, monkeypatch):
    ...
    fake_bookorbit_svc.set_secret_value("kindle_stk.oauth_state.code_verifier", "fake_verifier")
    ...
    assert (
        fake_bookorbit_svc.get_secret_value("kindle_stk.amazon_customer_id") == "amzn1.account.xyz"
    )
...
def test_set_default_destination_validates_against_device_list(fake_bookorbit_svc, monkeypatch):
    ...
    assert (
        fake_bookorbit_svc.get_secret_value("kindle_stk.default_destination_name")
        == "Kindle for Web"
    )
...
def test_send_file_translates_vendored_409_to_rate_limited(
...
def test_create_oauth_url_always_returns_amazon_com_for_india():
    ...
    assert "amazon.in" not in url.split("?")[0], f"URL base must NOT contain amazon.in: {url}"
