...
def test_router_picks_smtp_when_stk_not_configured(monkeypatch, db, cfg, book, file_path, svc):
    smtp_calls = []
    monkeypatch.setattr(
        "endless_library.kindle_router._send_smtp", lambda *a, **kw: smtp_calls.append(1)
    )
...
def test_router_picks_stk_when_configured(monkeypatch, db, cfg, book, file_path, svc):
    ...
    monkeypatch.setattr(
        "endless_library.kindle_stk.service.KindleStkService.send_file", fake_stk_send
    )
    from endless_library.kindle_router import DeliveryMethod, deliver
...
def test_router_falls_to_smtp_when_quota_exhausted(monkeypatch, db, cfg, book, file_path, svc):
    ...
    smtp_called = []
    monkeypatch.setattr(
        "endless_library.kindle_router._send_smtp", lambda *a, **kw: smtp_called.append(True)
    )
    from endless_library.kindle_router import DeliveryMethod, deliver
...
def test_router_records_send_stk_failed_event_on_exhaustion(
